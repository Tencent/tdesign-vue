...
onPresetClick | Function |  | TS 类型：`(context: { preset: PresetDate, e: MouseEvent, label: string }) => void`<br/>点击预设按钮后触发 | N
...
preset-click | `(context: { preset: PresetDate, e: MouseEvent, label: string })` | 点击预设按钮后触发
