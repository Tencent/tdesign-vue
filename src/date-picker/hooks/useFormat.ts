...
export default function useFormat(props: formatProps): formatResult {
  ...
  if (props.enableTimePicker) {
    ...
    if (!extractTimeFormat(valueType) && valueType !== 'time-stamp') console.error(`valueType: ${valueType} 不规范，包含时间选择必须要有时间格式化 HH:mm:ss`);
  }
...
function formatRange({
  newDate, format, targetFormat, sortType = 'min',
}: any) {
