...
export default function useRangeValue(props: TdDateRangePickerProps) {
  ...
  if (props.enableTimePicker) {
    ...
    if (!extractTimeFormat(formatRef.value.valueType) && formatRef.value.valueType !== 'time-stamp') console.error(`valueType: ${formatRef.value.valueType} 不规范，包含时间选择必须要有时间格式化 HH:mm:ss`);
  }
  ...
  const time = ref(initYearMonthTime(value.value, props.mode, formatRef.value.format, formatRef.value.timeFormat).time);
  const month = ref(initYearMonthTime(value.value, props.mode, formatRef.value.format).month);
  const year = ref(initYearMonthTime(value.value, props.mode, formatRef.value.format).year);
