...
checked | Boolean | - | 是否选中。支持语法糖 `v-model` | N
...
value | String / Number / Boolean | undefined | 单选按钮的值。TS 类型：`RadioValue` `type RadioValue = string | number | boolean`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/radio/type.ts) | N
...
value | String / Number / Boolean | undefined | 选中的值。支持语法糖 `v-model`。TS 类型：`RadioValue` | N
