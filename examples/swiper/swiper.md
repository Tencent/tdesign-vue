...
onChange | Function |  | 轮播切换时触发。`type SwiperChangeSource = 'autoplay' | 'click'`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/swiper/type.ts)。`(current: number, context: { source: SwiperChangeSource }) => {}` | N

### Swiper Events

名称 | 参数 | 描述
-- | -- | --
change | `(current: number, context: { source: SwiperChangeSource })` | 轮播切换时触发。`type SwiperChangeSource = 'autoplay' | 'click'`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/swiper/type.ts)
