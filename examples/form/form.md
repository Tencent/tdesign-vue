...
validateMessage | Object | - | 校验信息提示，主要用于非组件内部的校验信息呈现，如：表单初次呈现的远程校验结果。如果要启动组件内部的校验功能，该值必须设置为空。TS 类型：`FormValidateMessage` `interface FormValidateMessage { [field in keyof FormData]: Array<{ type: 'warn' | 'error'; message: string }>}`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/form/type.ts) | N
