...
onChange | Function |  | 值变化时触发。`interface ChangeContext { type: ChangeSource; e: InputEvent | MouseEvent | FocusEvent }`。`type ChangeSource = 'add' | 'reduce' | 'input' | ''`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/input-number/type.ts)。`(value: number, context: ChangeContext) => {}` | N
...
change | `(value: number, context: ChangeContext)` | 值变化时触发。`interface ChangeContext { type: ChangeSource; e: InputEvent | MouseEvent | FocusEvent }`。`type ChangeSource = 'add' | 'reduce' | 'input' | ''`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/input-number/type.ts)
