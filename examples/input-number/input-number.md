...
format | Function | - | 指定输入框展示值的格式。如果组件包含参数 `decimalPlaces`，参数会包含小数位数格式化之后的数据 `context.fixedNumber`。TS 类型：`(value: InputNumberValue, context?: { fixedNumber?: InputNumberValue }) => InputNumberValue` | N
