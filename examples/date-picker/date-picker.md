...
onPick | Function |  | 选中日期时触发，可能是开始日期，也可能是结束日期，第二个参数可以区分是开始日期或是结束日期。`interface PickContext { e: MouseEvent; partial: DateRangePickerPartial }`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/date-picker/type.ts)。`(value: DateValue, context: PickContext) => {}` | N
...
pick | `(value: DateValue, context: PickContext)` | 选中日期时触发，可能是开始日期，也可能是结束日期，第二个参数可以区分是开始日期或是结束日期。支持语法糖。`interface PickContext { e: MouseEvent; partial: DateRangePickerPartial }`。[详细类型定义](https://github.com/Tencent/tdesign-vue/tree/develop/src/date-picker/type.ts)
