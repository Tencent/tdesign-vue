...
<template>
  <base-usage :code="usageCode" :config-list="configList" :panel-list="panelList" @PanelChange="onPanelChange">
    <template #textarea="{ configProps }">
      <t-textarea placeholder="请输入内容" v-bind="configProps" />
    </template>
  </base-usage>
</template>
...
const usageCodeMap = { textarea: '<t-textarea placeholder="请输入内容" v-bind="configProps" />' };
